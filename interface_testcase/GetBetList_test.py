...
class GetBetList(unittest.TestCase):
    '''获取下注信息'''

    def setUp(self):
        self.baseurl = encrypt_oper.getURL('interfaceurl', 'getBetList')
    ...
    def test_get_player_order(self):
        ...
        self.teroutput.info("验证获取玩家下注信息:" + self.result['text'])
        self.fileoutput.info("验证获取玩家下注信息:" + self.result['text'])
    ...
    def test_get_EmpytOrder(self):
        ...
        params = {'agent': 'auto_agent', 'username': 'guyuanqiang', 'token': tokenVal,'deadline': ''}
        r = requests.post(url=self.baseurl, data=params)
        self.result = r.json()
